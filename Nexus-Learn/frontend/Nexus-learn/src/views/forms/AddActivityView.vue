...
const submit = async () => {
  ...
  try {
    // --- THE FIX IS HERE ---
    // We explicitly set Content-Type to 'multipart/form-data'.
    // Axios is smart enough to detect this and append the correct 'boundary',
    // overriding the 'application/json' default from api.js.
      ...
      <CardContent>
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <Input v-model="form.name" placeholder="e.g. Hackathon 2026" class="bg-white/5 border-white/10 text-white" required />
          </div>
