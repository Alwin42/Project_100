...
const cardVariants = {
  offscreen: { y: 300, opacity: 0, scale: 0.9 },
  onscreen: {
    y: 50,
    rotate: -10,
    opacity: 1,
    ...
    <div class="container mx-auto px-6 py-20 pb-40 relative z-10">
      ...
      <div class="text-center mb-24 space-y-6">
        ...
        <p class="text-xl text-gray-400 font-light tracking-wide">
          Your entire academic life, <span class="text-white font-semibold">stacked.</span>
        </p>
      </div>
      ...
      <div class="flex flex-col items-center w-full relative gap-12">
        ...
        <Motion 
          ...
          :whileInView="{ y: 0, opacity: 1, scale: 1, transition: { type: 'spring', bounce: 0.3 } }"
          :viewport="{ amount: 0.2 }"
          :style="{ 
            position: 'sticky', 
            top: `${110 + (index * 25)}px`, 
        ...
        >
          <div class="relative overflow-hidden rounded-[2rem] bg-black/40 backdrop-blur-2xl border border-white/10 shadow-2xl transition-all duration-500 ease-out group-hover:scale-[1.02] group-hover:border-white/20 group-hover:shadow-[0_0_50px_-12px_rgba(255,255,255,0.1)] h-[500px] flex flex-col">
            
            <div class="absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-700" 
                 :style="{ background: `radial-gradient(circle at top right, ${widget.color}, transparent 70%)` }">
            </div>
            
            <div class="absolute inset-0 opacity-10 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] pointer-events-none mix-blend-overlay"></div>

            <div class="relative z-10 p-8 flex flex-col h-full">
              
              <div class="flex items-center justify-between mb-8">
                <div class="flex items-center gap-4">
                  <div class="p-3.5 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-md text-white group-hover:bg-white/10 group-hover:text-nexus-accent transition-colors duration-300">
                    <component :is="widget.icon" class="w-7 h-7" :style="{ color: widget.color }" />
                  </div>
                  <h2 class="text-3xl font-bold text-white tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-400 transition-all">
                    {{ widget.title }}
                  </h2>
                </div>
              </div>
              ...
              <div v-if="widget.stat" class="mb-6 pb-6 border-b border-white/5">
                 ...
                 <div class="text-5xl font-black text-white mt-1 tracking-tight" 
                      :style="{ textShadow: `0 0 30px ${widget.color}40` }">
                    {{ widget.stat }}
                 </div>
              </div>

              <div class="space-y-3 flex-1">
                 <div v-for="(item, i) in widget.items" :key="i" 
                      class="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 transition-all duration-300 cursor-default group/item">
                    <span class="font-semibold text-gray-200 truncate group-hover/item:text-white">{{ item.main }}</span>
                    <span class="text-xs font-mono text-gray-500 group-hover/item:text-nexus-accent transition-colors">{{ item.sub }}</span>
                 </div>
                 
                 <div v-if="widget.items.length === 0" class="text-center text-gray-500 py-6 italic border border-dashed border-white/5 rounded-xl">
                    No items yet
                 </div>
              </div>
              ...
              <div class="mt-8">
                 <RouterLink :to="widget.link">
                    <Button class="w-full h-14 rounded-xl bg-white/5 hover:bg-white text-white hover:text-black font-bold border border-white/10 hover:border-white transition-all duration-300 text-lg group/btn shadow-lg backdrop-blur-sm">
                      Open {{ widget.title }} 
...
<style scoped>
/* Stacking Shadow Logic */
.sticky-card {
  /* Creates the "depth" shadow that falls on the card below it */
